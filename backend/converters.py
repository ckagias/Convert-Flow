...
─────────────────────────────────────────────────────────────────
Centralised file conversion logic for ConvertFlow.

All functions operate strictly inside UPLOAD_DIR / OUTPUT_DIR and
never trust incoming filenames. Callers must provide a stored file
name that was generated by the backend (UUID based).
"""
...
def docx_to_pdf(stored: str) -> ConversionResult:
  # Simple approximation: render each paragraph as text into a new PDF using PyPDF2.
  # This is NOT layout perfect but avoids unsafe shelling out to LibreOffice.
  input_path = UPLOAD_DIR / stored
  ...
  try:
    ...
    # PyPDF2 does not have high‑level text drawing; in a real system we would
    # use a richer PDF library. Here we just create an empty page to keep the
    # example minimal and safe.
...
def pptx_to_pdf(stored: str) -> ConversionResult:
  # Placeholder: create an empty PDF with one page.
  input_path = UPLOAD_DIR / stored
...
def pptx_to_images(stored: str, fmt: Literal["jpg", "png"]) -> ConversionResult:
  # Re‑use pdf2image pipeline via an intermediate PDF.
